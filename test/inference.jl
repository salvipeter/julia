...
@test all(e.kind === Core.Inference.INVALID_CALL_ARG for e in errors)
...
m0.sig = old_sig
@test length(errors) == 1
@test errors[1].kind === Core.Inference.SIGNATURE_LENGTH_MISMATCH

# SIGNATURE_VARARG_MISMATCH
m0.isva = false
errors = Core.Inference.validate_code(m0)
m0.isva = true
@test length(errors) == 1
@test errors[1].kind === Core.Inference.SIGNATURE_VARARG_MISMATCH
...
# m0.nargs -= 20
# @test length(errors) == 2
# @test any(e.kind == 11 for e in errors)
# @test any(e.kind == 14 for e in errors)
